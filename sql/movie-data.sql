...
select     *from    movie_cruiser.movie;
    
    
-- 2. View Movie List Customer (MVUC002)
-- a. Retrieving records (customer view)

select     *from    movie_cruiser.movie where mv_active = 'Yes' and mv_date_of_launch < (select CurDate());


-- 3. Edit Movie List (MVUC003)
-- a. Retrieving particular movie details for edit
select     *from    movie_cruiser.movie where    mv_id = 1;

-- b. Updating movie id=1
update movie_cruiser.movie set mv_title = 'FF9' ,mv_box_office = '1234567892' , mv_active = 'No' , 
mv_date_of_launch = '2018-03-15' , mv_genre = 'Superhero' , mv_has_teaser = 'No' where mv_id = 1;
